...
import {
  Background, NavBar, Button, Text, Form, SecureCodeDisplay, PhoneNumberInput,
} from 'components';
...
const ShareSecureCode = ({ navigation, route }) => {
  const dispatch = useDispatch();
  const { params: { type } } = route;

  let loadingAction = Actions.FETCH_SECURE_CODE_REQUEST;
  ...
  const isLoading = useSelector(Selectors.createLoadingSelector([
    loadingAction,
    Actions.SHARE_SECURE_CODE_REQUEST,
  ]));
      ...
      >
        <SafeAreaView style={styles.container}>
          <Text style={styles.margin} size="regular" weight="regular" centered>
            Enter the phone number you would like to share the Passcode.
          </Text>
          <SecureCodeDisplay type={type} style={styles.margin} codeFontSize="xxxlarge" errorFontSize="xxlarge" />
          <Formik
            validationSchema={validationSchema}
            onSubmit={(values) => {
              const { params: { boxId } } = route;
              dispatch(Actions.shareSecureCode(boxId, `${values.countryCode}${values.phoneNumber}`, type));
            }}
            initialValues={{
              phoneNumber: '',
...
ShareSecureCode.propTypes = {
  navigation: PropTypes.object.isRequired,
  route: PropTypes.object.isRequired,
};

export default ShareSecureCode;
